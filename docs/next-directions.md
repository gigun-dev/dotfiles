...
- **スマホから mini-vm の Codex を使える** (2026-09-01)。`codex-remote-control.service` が
  chatgpt.com 側のリレーへ**アウトバウンドで**繋ぐので受け口が無い。ホストごと再起動しても
  復帰することを実測済み (2026-09-06 / `DF-33`)。上流の `daemon bootstrap` は使わない
  (standalone を毎時自己更新し flake.lock が真実でなくなる)。
...
- **繰り返している失敗の形**: **検知器自身が壊れていると、壊れていることに気づけない**
  ...
  nix を触らない PR では job が走らず success に見えるため)。「確認した」と書くときは
  確認手段が生きているかを別経路で疑う。→ `DF-12`

- **未検証**: apple/container の再起動後の復帰 → `DF-5`。
- **裁定待ち**: SPEC.md、CLAUDE.md「ディレクトリ構造」節、ADR の置き場所 → `DF-2` `DF-3` `DF-6`

## 着手順(次にやること)

**順序の理由**: **リストの順ではなく `DF-37` → `DF-35` → `DF-38` が先**。2026-09-06 に
mini-vm が自分で更新するようになったので、その周りの穴を先に塞ぐ。`DF-37` は無人の push が
`admin: true` のトークンで走っている状態の解消(段 1 は今すぐできる)、`DF-35` は黙って
止まったときに気づく手段、`DF-38` は業務コードと agenix の鍵の同居。以降は Cloudflare OS の
主線(`DF-13` `DF-24` `DF-22`)、その後は裁定待ちか随伴作業。
(nd-tasks.sh は末尾に追記するので、並びと着手順は一致しない。**手で並べ替えない**。)
...
- ~~`DF-33` mini-vm 再起動後に codex-remote-control が復帰し、スマホから繋がるか確認する~~ ✅ 2026-09-06
      **自動更新の再起動を解禁する前提**(mini-vm.nix がこの ID を参照している)。詳細はカタログ。
      ...
      → 2026-09-06 に macOS ホストごと再起動して実測。機械側は全て合格 (Lima VM が autostart + 自動ログインで自動起動、常駐 5 サービスが手を触れず active、timer 再登録、健全性ゲート 4 項目通過、公開エンドポイントは再起動前と同じ、enrollment と control socket も残存)。**スマホの ChatGPT アプリから mini-vm が見え、接続済みになることも本人が確認**。これで自動更新の再起動解禁の前提が揃った
